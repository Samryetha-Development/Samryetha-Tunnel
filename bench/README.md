...
隧道每次转发都新建一条到本地服务的 TCP 连接，在高频请求下会产生大量握手与 TIME_WAIT。
客户端改为**连接池 + keep-alive 复用**（仅在对端愿意保持且能精确判定报文结尾时复用，否则回退并重试）。

对照实验：`bash bench/run-keepalive.sh`（600 次隧道请求，本地服务分别为 HTTP/1.0 与 HTTP/1.1）：

| 本地服务 | 请求数 | 本地新建连接 | 复用命中 |
|---|---|---|---|
| HTTP/1.0（关连接，等价旧行为） | 600 | **592** | 0 |
| HTTP/1.1 + 连接池 | 600 | **31** | 592 |

→ 本地连接创建数 **−95%**，握手与 TIME_WAIT 大幅减少。

正确性：600 次请求逐条校验响应体，`ok=600 bad=0`（复用不会串包）；SSE 流式不受影响。
安全性：`Content-Length` / `chunked` 精确判长才复用；`Connection: close`、HTTP/1.0 无 keep-alive、close-delimited 一律不复用；复用到已失效连接会自动换新连接重试一次。

> 回环上 p50 无可见变化（本地握手仅 ~20µs），收益体现在连接数/CPU/端口耗用，真实网络与高并发下更明显。
...
2. ~~**本地服务 keep-alive 连接池**~~ **已完成**：本地连接创建数 −95%，600 次请求逐条校验无串包。
